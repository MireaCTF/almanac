...
После анализа исполняемого файла, ида перенесет нас прямиком в ```WinMain```, в котором начинается код самого приложения, в то время как гидра остановится на буквальной точке входа, где распологается код сгенерированный компилятором для всяких там инициализаций и переноса потока выполнения в функцию ```WinMain``` (```main```).
